  ...
  <v-row class="mx-3">
    ...
    <v-col cols="6">
      ...
      <v-row>
        <v-col cols="12">
          <div
            style="font-size:1.8em;border-color:black;border-style:rounded"
            ref="editor"
            class="editor"
    ...
    <v-col cols="6">
      <v-row>
        <!--v-col cols="12" v-if="!alert.alerts.length">
          <v-img src="../assets/mr_clippy.gif" class="mx-auto"></v-img>
        </v-col-->
      </v-row>
      <v-row style="width: 700px;">
        <v-expansion-panels>
          <v-expansion-panel v-for="(item, i) in alert.alerts" :key="i">
            <v-expansion-panel-header>
              <v-alert color="cyan" border="left" elevation="0" colored-border>{{ item.help }}</v-alert>
            </v-expansion-panel-header>
            <v-expansion-panel-content style="margin-left:50px">
              <v-alert color="deep-purple accent-4" border="left" elevation="0" colored-border>
                <div v-html="item.detailed"></div>
                <v-divider class="my-4 info" style="opacity: 0.22"></v-divider>
                <!--<div>
                  {{ item.help }}
                  <v-btn color="info" outlined @click="resolve(item)">Okay</v-btn>
                </div>-->
              </v-alert>
...
export default {
  ...
  data: () => ({
    ...
    argument: "",
    loading: false,
    topicSelect: { label: "School Uniforms: Good or Bad?", value: "uniforms" },
    topics: [
  ...
  methods: {
    fetchFeedback(evt) {
      axios
        .get(
          `/api/feedback/?argument=${this.argument}&topic=uniforms`
	  //`/api/feedback/?argument=${this.argument}&topic=${this.topicSelect.value}`
        )
        .then(response => {
        ...
        .catch(error => {
          console.log(error);
          this.error = error;
          //this.showAlert("Backend errored: " + error);
        })
    ...
    resolve(item) {
      ...
      console.log(el);
      console.log(el.parentElement.classList.remove("error"));
      el.outerHTML = item.action;
      this.alert.alerts = this.alert.alerts.filter(obj => obj.id !== item.id);
    }
  }
};
</script>
